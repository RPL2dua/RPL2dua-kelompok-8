...
def predict():
    if (path == ""):
        ...
        model = load_model('C:\\Users\\ikhram\\PycharmProjects\\PendeteksiHelm\\model.h5')

        labels = ['Helm ON', 'NO Helm']  # label ini berguna saat menghasilkan output
        start_point = (15, 15)
        end_point = (370, 80)
        thickness = -1
...
Title_Label = Label(Tops, font=('Comic Sans MS', 20, 'bold'),
                    text="Pendeteksi Helm \n\t\t Menggunakan Machine Learning\t\t", pady=9, bg='white',
                    fg='red', justify="center")
Title_Label.grid(row=0, column=0)
MainFrame = Frame(root, bg='white', pady=2, padx=2, width=1350, height=100, relief=RIDGE)
MainFrame.grid(row=1, column=0)

Label_1 = Label(MainFrame, font=('lato black', 17, 'bold'), text="\t\t\tDetect helmet", padx=2, pady=2, bg="white",
                fg="black")
...
Label_9 = Button(MainFrame, font=('arial', 19, 'bold'), text="  Pilih Gambar ", padx=2, pady=2, bg="blue", fg="white",
                 command=select_image)
Label_9.grid(row=2, column=0)

Label_9 = Button(MainFrame, font=('arial', 19, 'bold'), text="  Deteksi Helm ", padx=2, pady=2, bg="blue", fg="white",
                 command=predict)
...
Label_3 = Label(MainFrame, font=('arial', 10, 'bold'), text="\t\t\t\t          ", padx=2, pady=2, bg="white",
                fg="black")
